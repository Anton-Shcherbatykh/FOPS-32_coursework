...
provider "yandex" {
  token     = "y0__xCp2N8GGMHdEyCmue-9FPv5_idz2IGL_LzU2YgUilP38A9L"
  cloud_id  = "b1gnnusp3dporth6taks"
  folder_id = "b1gforq1ri2he906c86d"
  zone      = "ru-central1-a"
}
...
#################################################################################
resource "yandex_vpc_network" "network-course-project" {
  name        = "network-course-project"
  description = "Network course project DevOps engineer"
}

###########################################################################################################
## Subnet. Gateway. Route table ###### https://cloud.yandex.ru/ru/docs/vpc/operations/create-nat-gateway ##
###########################################################################################################
resource "yandex_vpc_subnet" "a-subnet-course-project" {
  name           = "a-subnet-course-project"
  v4_cidr_blocks = ["192.168.60.0/24"]
  zone           = "ru-central1-a"
  network_id     = yandex_vpc_network.network-course-project.id
  route_table_id = yandex_vpc_route_table.a-b-subnet-route-table.id
}

resource "yandex_vpc_subnet" "b-subnet-course-project" {
  name           = "b-subnet-course-project"
...
resource "yandex_vpc_subnet" "d-subnet-course-project" {
  name           = "d-subnet-course-project"
...
resource "yandex_vpc_security_group" "bastion-security-local" {
  ...
  ingress {
    ...
    v4_cidr_blocks = ["192.168.60.0/24", "192.168.70.0/24", "192.168.80.0/24"]
    port           = 22
  }
...
resource "yandex_compute_instance" "bastion" {
  name  = "bastion"
  hostname = "bastion"
  zone  = yandex_vpc_subnet.d-subnet-course-project.zone
  ...
  network_interface {
    subnet_id = "${yandex_vpc_subnet.d-subnet-course-project.id}"
    nat = true
    ipv4 = true
...
resource "yandex_compute_instance" "nginx-web-1" {
  ...
  zone  = yandex_vpc_subnet.a-subnet-course-project.zone
  platform_id     = "standard-v3"
  resources {
    cores         = 2
    core_fraction = 20
  ...
  network_interface {
    subnet_id = "${yandex_vpc_subnet.a-subnet-course-project.id}"
    ipv4 = true
...
resource "yandex_compute_instance" "nginx-web-2" {
  ...
  zone  = yandex_vpc_subnet.b-subnet-course-project.zone
  platform_id     = "standard-v3"
  resources {
    cores         = 2
    core_fraction = 20
  ...
  network_interface {
    subnet_id = "${yandex_vpc_subnet.b-subnet-course-project.id}"
    ipv4 = true
...
resource "yandex_alb_load_balancer" "nginx-balancer" {
  ...
  allocation_policy {
    location {
      zone_id   = "${yandex_vpc_subnet.d-subnet-course-project.zone}"
      subnet_id = yandex_vpc_subnet.d-subnet-course-project.id
    }
  }
...
#################################################################################################################
resource "yandex_compute_snapshot_schedule" "snapshot-course-project" {
  name = "snapshot-course-project"
  ...
  snapshot_spec {
      description = "Snapshots. Every day at 01:50"
  }
